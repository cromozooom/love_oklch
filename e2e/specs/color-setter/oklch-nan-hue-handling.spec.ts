...
test.describe('OKLCH NaN Hue Handling', () => {
  ...
  test('should handle grey colors without NaN in OKLCH format', async ({ page }) => {
    ...
    for (const greyColor of greyColors) {
      ...
      const displayValue = await page.locator('[data-testid="display-value"]').textContent();
      expect(displayValue).not.toContain('NaN');
      console.log(`Display value: ${displayValue}`);
      
      // If it switched to OKLCH, verify the format is valid
      const activeFormat = await page.locator('[data-active="true"]').textContent();
      if (activeFormat?.includes('OKLCH')) {
        // OKLCH format should be oklch(L C H) where all values are numbers
        expect(displayValue).toMatch(/oklch\(\d+\.\d+\s+\d+\.\d+\s+\d+(\.\d+)?\)/);
        expect(displayValue).not.toContain('NaN');
      }
  ...
  test('should handle achromatic colors in OKLCH sliders without NaN', async ({ page }) => {
  ...
  test('should maintain valid OKLCH format when switching between grey colors', async ({ page }) => {
    // Switch to OKLCH format
    await page.click('[data-testid="format-selector-oklch"]');
